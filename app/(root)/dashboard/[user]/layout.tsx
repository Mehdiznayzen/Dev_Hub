...
const AuthLayout = ({ active, title, subtitle, searchPlaceholder, searchValue, onSearchChange, showAskButton, showSearch = false, children, rightPanel, }: PageShellProps) => {
  const [modalOpen, setModalOpen] = useState(false);
  const { user } = useUser();

  const activeLink =
    active === 'Questions'
      ? `/dashboard/${user?.id}/questions`
      : active === 'Tags'
      ? `/dashboard/${user?.id}/tags`
      : active === 'Users'
      ? `/dashboard/${user?.id}/users`
      : undefined;
  ...
  return (
    <div className="relative min-h-screen">
      ...
      <DashboardNavbar
        onAskQuestion={showAskButton ? () => setModalOpen(true) : undefined}
